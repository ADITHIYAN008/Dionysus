...
export async function summariseCode(doc: Document) {
  ...
  try {
    ...
    const response = await model.generateContent([
      ...
      `You are onboarding a junior software engineer and explaining to them the purpose of the ${doc.metadata.source} file Here is the code: --- ${code} --- Give a summary no more than 100 words of the code above`,
    ]);
    return response.response.text();
  } catch (error) {
    return "";
